...
static int decode_quoted(struct flb_parser_dec *dec,
                         char *in_buf, size_t in_size,
                         char **out_buf, size_t *out_size, int *out_type)
{
    int len;
    if(in_size < 2) {
        ...
        *out_type = TYPE_OUT_STRING;
        ...
        len = flb_unquote_string(in_buf+1, in_size-2, &dec->buffer);
        *out_buf = dec->buffer;
        *out_size = len;
        *out_type = TYPE_OUT_STRING;
    }
                          ...
                          char **out_buf, size_t *out_size)
{
    ...
    for (i = 0; i < map.via.map.size; i++) {
        ...
        mk_list_foreach(r_head, &dec->rules) {
            ...
            else if (rule->backend == FLB_PARSER_DEC_QUOTED) {
                ret = decode_quoted(dec,
                                    (char *) data_sds, flb_sds_len(data_sds),
                                    &dec_buf, &dec_size, &dec_type);
            }
...
struct mk_list *flb_parser_decoder_list_create(struct mk_rconf_section *section)
{
    ...
    mk_list_foreach(head, &section->entries) {
        ...
        else if (strcasecmp(decoder->value, "quoted") == 0) {
            backend = FLB_PARSER_DEC_QUOTED;
        }
        else {
